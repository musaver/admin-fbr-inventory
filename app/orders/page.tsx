...
export default function OrdersList() {
  ...
  const handleDeleteAll = async () => {
    if (confirm('Are you sure you want to delete ALL SANDBOX orders? Production orders will NOT be affected. This action cannot be undone and will also delete all related data including order items, stock movements, and loyalty points history.')) {
      try {
        setLoading(true);
        const res = await fetch('/api/orders/delete-all', { method: 'DELETE' });
        if (res.ok) {
          const prodOrders = orders.filter(o => o.fbrEnvironment === 'production');
          setOrders(prodOrders);
          setFilteredOrders(prodOrders);
          setSelectedOrders(new Set());
          setSelectAll(false);
          alert('All sandbox orders have been deleted successfully. Production orders are preserved.');
        } else {
        ...
        alert('Error deleting sandbox orders. Please try again.');
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleSelectAll = (checked: boolean) => {
    ...
    if (checked) {
      const selectableOrders = filteredOrders.filter(order => order.fbrEnvironment !== 'production');
  ...
  const handleSelectOrder = (orderId: string, checked: boolean) => {
    ...
    if (order?.fbrEnvironment === 'production') return;
    ...
    const selectableOrders = filteredOrders.filter(o => o.fbrEnvironment !== 'production');
    setSelectAll(newSelected.size === selectableOrders.length && selectableOrders.length > 0);
  };

  const handleDeleteSelected = async () => {
    if (selectedOrders.size === 0) return;

    const safeIds = Array.from(selectedOrders).filter(id => {
      const order = orders.find(o => o.id === id);
      return order?.fbrEnvironment !== 'production';
    });

    if (safeIds.length === 0) {
      alert('No sandbox orders selected. Production orders cannot be deleted.');
      return;
    }

    if (confirm(`Are you sure you want to delete ${safeIds.length} selected sandbox order(s)? This action cannot be undone.`)) {
      try {
        setLoading(true);
        const deletePromises = safeIds.map(orderId =>
        ...
        if (failedDeletes.length === 0) {
          const deletedSet = new Set(safeIds);
          const remainingOrders = orders.filter(order => !deletedSet.has(order.id));
          ...
          setSelectedOrders(new Set());
          setSelectAll(false);
          alert(`${safeIds.length} sandbox order(s) deleted successfully.`);
        } else {
  ...
  const columns = [
    {
      ...
      render: (_: any, order: Order) => {
        const isProd = order.fbrEnvironment === 'production';
        return (
          <input
            type="checkbox"
            ...
            disabled={isProd}
            className={`rounded border-gray-300 text-blue-600 focus:ring-blue-500 ${isProd ? 'opacity-30 cursor-not-allowed' : ''}`}
            title={isProd ? 'Production orders cannot be deleted' : undefined}
          />
        );
      }
    },
        ...
        <div className="flex flex-col space-y-2 flex-shrink-0">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Button
              ...
              disabled={loading || orders.filter(o => o.fbrEnvironment !== 'production').length === 0}
              variant="destructive"
              size="sm"
            >
              <TrashIcon className="h-4 w-4 mr-2" />
