...
function parseOrderCSV(csvText: string): OrderImportRow[] {
  ...
  const columnMap = {
    ...
    'lotNumber': ['lot number', 'lot_number', 'lot no'],
    'expiryDate': ['expiry date', 'expiry_date', 'expiration date'],
    'itemSerialNumber': ['item serial number', 'item_serial_number', 'item serial no'],
    'sroScheduleNumber': ['sro/schedule number', 'sro schedule number', 'sro_schedule_number', 'sro schedule no']
  };
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const orderData: OrderImportRow = {
      ...
      lotNumber: values[headerMap.lotNumber] || '',
      expiryDate: values[headerMap.expiryDate] || '',
      itemSerialNumber: values[headerMap.itemSerialNumber] || '',
      sroScheduleNumber: values[headerMap.sroScheduleNumber] || '',
    };
